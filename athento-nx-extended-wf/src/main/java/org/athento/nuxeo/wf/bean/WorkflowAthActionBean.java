...
public class WorkflowAthActionBean implements Serializable {
    ...
    public void observerAndRaiseForEvents() {
        ...
        EventContext eventContext = new DocumentEventContext(documentManager,
                ...
                document);
        if (taskDoc == null) {
            WorkflowUtils.newEntry(document, documentManager.getPrincipal().getName(),
                    "Workflow changed", WorkflowExtConstants.WORKFLOW_CATEGORY, "Ended, rejected, abandoned or canceled by user", null, null);
            return;
        }
    ...
    /**
     * Check if a task is collapsible.
     *
     * @param taskId
     * @return
     */
