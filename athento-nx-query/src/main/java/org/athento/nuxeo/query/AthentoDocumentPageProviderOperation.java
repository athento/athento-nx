 ...
 *
 * @author Victor Sanchez (vs@athento.com) based on Tiry (tdelprat@nuxeo.com)
 * @since 8.10+
 */
@Operation(id = AthentoDocumentPageProviderOperation.ID, category = Constants.CAT_FETCH, label = "PageProvider", description = "Perform "
...
public class AthentoDocumentPageProviderOperation {
    ...
    private static final Log log = LogFactory.getLog(AthentoDocumentPageProviderOperation.class);
    ...
    public PaginableDocumentModelListImpl run() throws OperationException {
        ...
        if (query != null) {
            // Default in ES use LIKE always
            query = query.replace("?", (String) parameters[0]);
            ...
            // Execute queries
            List<DocumentModel> result = QueryUtils.executeQuery(session, query, targetOffset, targetLimit, sortInfoList);
            PageProvider<DocumentModel> pp = new DocumentModelListPageProvider(new DocumentModelListImpl(result));
    ...
    private Object[] resolveParameters(Object[] parameters) {
        ActionContext actionContext = (ActionContext) context.get(SEAM_ACTION_CONTEXT);
